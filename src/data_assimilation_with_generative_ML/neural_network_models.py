...
class TransformerBlock(nn.Module):
    def __init__(self, seq_len, dim, heads, mlp_dim, k, rate=0.0):
        ...
        # self.attn = nn.MultiheadAttention(dim, heads, dropout=rate)
    ...
    def forward(self, x, c):
        ...
        x_modulate = modulate(self.ln_1(x), shift_msa, scale_msa)
        x = self.attn(x_modulate) * gate_msa + x
        # x_modulate, _ = self.attn(x_modulate, x_modulate, x_modulate)
        # x = x_modulate * gate_msa + x
