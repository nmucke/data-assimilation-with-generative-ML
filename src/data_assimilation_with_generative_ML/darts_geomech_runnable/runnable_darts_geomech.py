...
# Try to import DARTS modules
try:
    from darts.reservoirs.cpg_reservoir import read_arrays
except ImportError:
    print("Error importing DARTS modules. Make sure DARTS is installed and in your Python path.")
    sys.exit(1)
...
def get_case_files(case: str, physics_case: str) -> tuple:
    ...
    possible_dirs = [
        ...
        os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'),  # Parent directory of this script
        '/samoa/data/smrserraoseabr/data-assimilation-with-generative-ML/src/data_assimilation_with_generative_ML'  # Hardcoded path
    ]

    for base_dir in possible_dirs:
        prefix = os.path.join(base_dir, 'meshes', case)
        ...
        if all(os.path.exists(file) for file in [gridfile, propfile, sch_file]):
            print(f"Files found in: {prefix}")
            return gridfile, propfile, sch_file
    
    # If we've searched all directories and haven't found the files, raise an error
    raise FileNotFoundError(f"Could not find required files for case '{case}' and physics_case '{physics_case}' in any of the searched directories. "
                            f"Searched in:\n" + "\n".join(possible_dirs) + 
                            f"\nExpected files:\n{gridfile}\n{propfile}\n{sch_file}")
