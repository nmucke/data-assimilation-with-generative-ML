...
import numpy as np
import xarray as xr
from data_assimilation_with_generative_ML.bayesian_inference.langevin_dynamics import compute_langevin_samples
...
def main():
    ...
    latent_langevin = compute_langevin_samples(
        log_prob,
        latent_vec,
        num_iterations=1,
        lr=1e-1,
    ...
    latent_langevin = latent_langevin.reshape(N, 1, 64, 64)

    with torch.no_grad():
        state_langevin = sampling_model(latent_langevin)

    state_langevin_obs = torch.zeros(N, 2, num_obs_X, num_obs_X, 121)
    for i in range(N):
        state_langevin_obs[i] = observation_operator(state_langevin[i:i+1])

    state_langevin = torch.mean(state_langevin, dim=0)

    latent_langevin = latent_langevin.reshape(N, 1, 64, 64)
    pars_langevin = ode_sampler(
        z=latent_langevin.to(device),
        score_model=score_model.to(device), 
        marginal_prob_std=marginal_prob_std_fn, 
        diffusion_coeff=diffusion_coeff_fn, 
        num_steps=15,
    ...
    pars_langevin = pars_langevin.mean(axis=0).unsqueeze(0)

    plot_monte_carlo_results(
        state_true=state_true,
        state_map=state_map[0:1],
        state_monte_carlo=state_langevin,
        state_obs_true=state_obs_true[0],
        state_map_obs=state_map_obs[0],
        state_monte_carlo_obs=state_langevin_obs,
        pars=pars,
        pars_map=pars_map,
        pars_monte_carlo=pars_langevin,
        X=X,
        Y=Y,
        plot_path='langevin_inverse'
    )
